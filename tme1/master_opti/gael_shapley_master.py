...
def gael_shapley_master(fichier1, fichier2):
    """
    cette fonction implemente l'algorithme des hopitaux, cote master.
    On utilisera 2 listes FIFO, une file d'attente pour les masters et une autre file separee 
    pour leurs demandes.
    elle retourne la liste des tuples parfaits et stables, version master-optimal, sous forme de
    ...
    #on a besoin de savoir quels etudiants sont libres a tout instant
    ...
    while(len(liste_demandeurs)>0): 
        ...
        #on recupere l'etudiant demande    (toujours le 1er de la liste)
        true_etu_demande=liste_demandeurs[0][1]
        etu_demande=utile.from_numero_to_etu(true_etu_demande)
        #on ajoute l'etudiant demande dans le dictionnaire des demandes (sans transformation)
        dict_demandes[true_master_demandeur].append(true_etu_demande)
        
        #si l'etudiant demande est libre
        if etu_demande in liste_etu_libre:
            ...
            #des demandes a un etudiant deja accepte
        ...
        #sinon
            ...
            #si l'etudiant prefere le master demandeur a celui qu'il a deja                                                
            if matrice_etu[indice_matrice_etu].index(master_demandeur) < matrice_etu[indice_matrice_etu].index(master_rival):
                #on enleve l'etudiant de la liste des acceptes du master rival
                ...
                #si le master rival a une place de libre et une seule,
                #ca veut dire qu'il etait absent de la liste des demandeurs, on le remet donc dedans
